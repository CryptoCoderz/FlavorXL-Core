...
{
    QString ret = QString("FlavorXL:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non FXL unit input in lnReqAmount, we generate the URI with FXL as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(FlavorXLUnits::format(FlavorXLUnits::FXL, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
