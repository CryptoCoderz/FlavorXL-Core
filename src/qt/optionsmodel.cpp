...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", FlavorXLUnits::FXL);
    ...
    if (!settings.contains("nAnonymizeFlavorXLAmount"))
        settings.setValue("nAnonymizeFlavorXLAmount", 1000);
    nAnonymizeFlavorXLAmount = settings.value("nAnonymizeFlavorXLAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeFlavorXLAmount"))
        SoftSetArg("-anonymizeFlavorXLamount", settings.value("nAnonymizeFlavorXLAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeFlavorXLAmount:
            return QVariant(nAnonymizeFlavorXLAmount);
        case UseDarkTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeFlavorXLAmount:
            nAnonymizeFlavorXLAmount = value.toInt();
            settings.setValue("nAnonymizeFlavorXLAmount", nAnonymizeFlavorXLAmount);
            emit AnonymizeFlavorXLAmountChanged(nAnonymizeFlavorXLAmount);
            break;
        default:
            break;
        }
