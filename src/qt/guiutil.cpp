...
{
    widget->setMaxLength(FlavorXLAddressValidator::MaxAddressLength);
    widget->setValidator(new FlavorXLAddressValidator(parent));
...
bool parseFlavorXLURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("FlavorXL"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!FlavorXLUnits::parse(FlavorXLUnits::FXL, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseFlavorXLURI(QString uri, SendCoinsRecipient *out)
{
    // Convert rev:// to FlavorXL:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("rev://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "FlavorXL:");
    }
    QUrl uriInstance(uri);
    return parseFlavorXLURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FlavorXL.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for FlavorXL.lnk
...
{
    return GetAutostartDir() / "FlavorXL.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a FlavorXL.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=FlavorXL\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the FlavorXL app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add FlavorXL app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("FlavorXL-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  FlavorXL-qt [" + tr("command-line options") + "]                     " + "\n";
