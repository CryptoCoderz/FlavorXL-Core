...
int nAnonymizeFlavorXLAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FXL
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FXL
    // Mac: ~/Library/Application Support/FXL
    // Unix: ~/.FXL
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FXL";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "FlavorXL.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "FlavorXL.conf";
               ...
               fprintf(ConfFile, "port=20299\n");
               fprintf(ConfFile, "rpcport=20099\n");
               fprintf(ConfFile, "rpcconnect=127.0.0.1\n");
               fprintf(ConfFile, "rpcallowip=127.0.0.1\n");
               fprintf(ConfFile, "addnode=172.105.121.51:20299\n");
               fprintf(ConfFile, "addnode=173.230.156.35:20299\n");
               fprintf(ConfFile, "addnode=172.104.168.50:20299\n");
               fclose(ConfFile);
    }
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "FlavorXLd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong FlavorXL will not work properly.");
