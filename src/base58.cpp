...
namespace {
    class CFlavorXLAddressVisitor : public boost::static_visitor<bool> {
    private:
        CFlavorXLAddress *addr;
    public:
        CFlavorXLAddressVisitor(CFlavorXLAddress *addrIn) : addr(addrIn) { }
...
bool CFlavorXLAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CFlavorXLAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CFlavorXLAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CFlavorXLAddressVisitor(this), dest);
}

bool CFlavorXLAddress::IsValid() const {
...
CTxDestination CFlavorXLAddress::Get() const {
    if (!IsValid())
...
bool CFlavorXLAddress::GetKeyID(CKeyID &keyID) const {
...
bool CFlavorXLAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CFlavorXLSecret::SetKey(const CKey& vchSecret) {
...
CKey CFlavorXLSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CFlavorXLSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CFlavorXLSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CFlavorXLSecret::SetString(const std::string& strSecret) {
